...
router.get("/admin/dashboard", async (_req, res) => {
  ...
  res.json(GetDashboardResponse.parse({
    totalProducts,
    availableProducts: rows.filter((r) => r.status === "available").reduce((s, r) => s + r.count, 0),
    soldOutProducts: rows.filter((r) => r.status === "sold-out").reduce((s, r) => s + r.count, 0),
    hiddenProducts: rows.filter((r) => r.status === "hidden").reduce((s, r) => s + r.count, 0),
    featuredProducts: rows.filter((r) => r.featured).reduce((s, r) => s + r.count, 0),
